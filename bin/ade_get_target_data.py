...
csv_map = [
    ...
    { 'field' : 'chembl_ligand_score', 'label': 'CHEMBL_LIGAND_SCORE'},
    { 'field' : 'chembl_pdb_count', 'label': 'CHEMBL_PDB_COUNT'},
    { 'field' : 'chembl_pdb_score', 'label': 'CHEMBL_PDB_SCORE'}
]
...
def get_ade_info(in_file, out_file):
    ...
    for gene in genes:
        ...
        if gene in gene_cache:
            ...
            logger.debug("\nHUGO: data_map: {} results: {}".format(data_map['HUGO'], h_data))
            if len(h_data) > 1:
                logger.critical("HUGO array of results > 1 for {} -- don't know how to handle".format(gene))
                break
            for mapping in data_map['HUGO']:
                gene_info[mapping['target_field']] = ''
                if mapping['source_field'] in h_data[0]:
                    value = h_data[0][mapping['source_field']]
                    ...
                    logger.debug("  data: {}".format(h_data[0][mapping['source_field']]))
            logger.debug("\n after HUGO gene_info: {}\n".format(gene_info))

            #PHAROS data - use HUGO gene symbol
            if gene_info['hgnc_symbol']:
                ...
                if len(p_data) == 1:
                    ...
                    for mapping in data_map['PHAROS']:
                        value = ''
                        if mapping['source_field'] in p_data[0]:
                            value = p_data[0][mapping['source_field']]
                        gene_info[mapping['target_field']] = value
                else:
            ...
            if gene_info['hgnc_symbol'] and gene_info['hgnc_uniprot_ids']:
                ...
                if ':' in uniprot_id:   # multiple uniprot_ids
                    ...
                    if len(targets):
                        ...
                        if target_chembl_id:
                            ...
                            logger.debug("\nChEMBL: data_map: {} results: {}\n".format(data_map['CHEMBL'], c_processed_data))
                            for mapping in data_map['CHEMBL']:
                                value = ''
                                if mapping['source_field'] in c_processed_data:
